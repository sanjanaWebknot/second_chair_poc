...
if COLLECTION_NAME in [c.name for c in chroma_client.list_collections()]:
    ...
    collection = chroma_client.create_collection(
        name=COLLECTION_NAME, metadata={"source": "second_chair_ws"}
    )
...
class QAPairBuffer:
    ...
    def add_word(self, word: str) -> List[Dict[str, Any]]:
        ...
        # Check for Q. pattern - handle both standalone and content cases
        if self._is_question_marker(word):
            ...
            if self.current_q and self.current_a and self.in_answer:
                ...
                print(
                    f"📝 PAIR_COMPLETE: {completed_pair['id']} | Q: {self.current_q[:30]}... | A: {self.current_a[:30]}..."
                )

                # Mark older pairs as ready (keep newest forming)
                ready_pairs.extend(self._mark_older_pairs_ready())

            # Start new question - extract content after "Q."
            if word.strip() == "Q.":
                # Standalone Q. marker
                self.current_q = ""
                self.current_a = ""
                ...
                # Q. with content on same line
                question_content = word[2:].strip()  # Remove "Q." and get content
                self.current_q = question_content
                self.current_a = ""
                self.in_answer = False

        # Check for A. pattern - handle both standalone and content cases
            ...
            if self.current_q:
                if word.strip() == "A.":
                    ...
                    # A. with content on same line
                    answer_content = word[2:].strip()  # Remove "A." and get content
                    self.current_a = answer_content
                    self.in_answer = True
    ...
    def _mark_older_pairs_ready(self) -> List[Dict[str, Any]]:
        ...
        ready_pairs = []
        forming_pairs = [
            p for p in self.pairs_buffer if p["state"] == PairState.FORMING
        ]
        ...
        # If buffer is getting very full, mark all forming pairs as ready
            ...
            for pair in forming_pairs:
                ...
                ready_pairs.append(pair)
                print(
                    f"🔄 PRESSURE_READY: {pair['id']} (buffer at {self.get_buffer_pressure():.1%})"
                )

        return ready_pairs

    def _is_question_marker(self, word: str) -> bool:
        """Check if word starts with Q."""
        return word.strip().startswith("Q.")
    
    def _is_answer_marker(self, word: str) -> bool:
        """Check if word starts with A."""
        return word.strip().startswith("A.")
    ...
    def get_ready_pairs(self) -> List[Dict[str, Any]]:
        ...
        return [
            p for p in self.pairs_buffer if p["state"] == PairState.READY_TO_PROCESS
        ]
...
async def get_transcript_session_status():
    ...
    async with session_lock:
        return {
            ...
            "session_id": transcript_session_id,
            "start_time": (
                transcript_start_time.isoformat() if transcript_start_time else None
            ),
    ...
    try:
        ...
        try:
            ...
            if not hasattr(verdict_response, "verdict"):
                print(
                    f"⚠️ WARNING: {pair_id} - Invalid verdict_response object, skipping"
                )
                return
            ...
            explanation = getattr(
                verdict_response, "explanation", "No explanation available"
            )
            ...
            try:
                ...
                if not (0 <= confidence <= 100):
                    print(
                        f"⚠️ WARNING: {pair_id} - Invalid confidence {confidence}, using 0"
                    )
                    confidence = 0
            ...
            try:
                ...
                print(
                    f"⚠️ WARNING: {pair_id} - JSON serialization error: {json_error}, skipping"
                )
                return
            ...
            if websocket.client_state.name == "CONNECTED":
                try:
                    ...
                    verdict_emoji = (
                        "✅"
                        if verdict == "SUPPORT"
                        else "❌" if verdict == "REFUTE" else "❓"
                    )
                    print(
                        f"{verdict_emoji} RESULT: {pair_id} -> {verdict} ({confidence}%) in {processing_time}ms"
                    )
            ...
            print(
                f"⚠️ WARNING: {pair_id} - Error parsing claim checker response: {parsing_error}, skipping"
            )
            return
                ...
                for pair in ready_pairs:
                    ...
                    try:
                        async with transcript_lock:
                            ...
                            print(
                                f"📄 APPENDED: {pair['id']} (seq: {pair['sequence_number']}) -> size: {len(running_transcript)}"
                            )
                    except Exception as e:
