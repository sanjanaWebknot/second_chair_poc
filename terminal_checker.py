...
# Configuration
DEFAULT_PDF_FILE = "Untitled document (1).pdf"
...
def setup_database(pdf_file=None, force_rebuild=False):
    ...
    for i, chunk in enumerate(chunks[:5]):
        print(f"\n--- CHUNK {i+1} ---")
        print(
            f"Page: {chunk.get('page')}, Lines: {chunk.get('start_line')}-{chunk.get('end_line')}"
        )
        print(f"Type: {chunk.get('type')}")
        print(f"Length: {len(chunk.get('clean_text', ''))} chars")
        print("Content:")
        print(
            f"'{chunk.get('clean_text', '')[:300]}{'...' if len(chunk.get('clean_text', '')) > 300 else ''}"
        )
    ...
    try:
        ...
        upsert_chunks_to_chroma(
            chroma_client, COLLECTION_NAME, chunks, embedder, pdf_file, verbose=True
        )
...
def check_claim(chroma_client, embedder, statement, top_k=5, model="phi3:mini"):
    ...
    for i, hit in enumerate(hits):
        ...
        print(
            f"    {i+1}. Page {page}, Lines {lines} ({qna_count} Q&A pairs, context: {context}, relevance: {relevance:.3f})"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Second Chair - Terminal Claim Checker"
    )
    parser.add_argument(
        "--pdf",
        "-p",
    ...
    print(
        "Commands: 'quit'/'exit' to stop, 'setup' to rebuild database, 'clear' to clear database"
    )
